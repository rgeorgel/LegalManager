...
// ─── ProcessosMonitoradosController – Search validation ──────────────────────

public class ProcessosMonitoradosControllerSearchTests
{
    private static ProcessosMonitoradosController CreateController()
    {
        var svc = new Mock<IProcessoMonitoradoService>();
        var dataJud = new DataJudAdapter(
            new HttpClient { BaseAddress = new Uri("http://localhost") },
            Mock.Of<ILogger<DataJudAdapter>>());
        var logger = Mock.Of<ILogger<ProcessosMonitoradosController>>();
        return new ProcessosMonitoradosController(svc.Object, dataJud, logger);
    }

    [Fact]
    public async Task Search_EmptyCNJ_ReturnsBadRequest()
    {
        var ctrl = CreateController();
        var result = await ctrl.Search("", null, default);
        Assert.IsType<BadRequestObjectResult>(result);
    }

    [Fact]
    public async Task Search_WhitespaceCNJ_ReturnsBadRequest()
    {
        var ctrl = CreateController();
        var result = await ctrl.Search("   ", null, default);
        Assert.IsType<BadRequestObjectResult>(result);
    ...
    [Fact]
    public async Task Search_TooFewDigits_ReturnsBadRequest()
    {
        var ctrl = CreateController();
        var result = await ctrl.Search("123456", null, default);
        Assert.IsType<BadRequestObjectResult>(result);
    }

    [Fact]
    public async Task Search_ToggleAsync_NotFound_ReturnsNotFound()
    {
        var svc = new Mock<IProcessoMonitoradoService>();
        svc.Setup(s => s.ToggleAtivoAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
           .ThrowsAsync(new KeyNotFoundException());
        var dataJud = new DataJudAdapter(
            new HttpClient { BaseAddress = new Uri("http://localhost") },
            Mock.Of<ILogger<DataJudAdapter>>());
        var ctrl = new ProcessosMonitoradosController(svc.Object, dataJud, Mock.Of<ILogger<ProcessosMonitoradosController>>());

        var result = await ctrl.Toggle(Guid.NewGuid(), default);
    ...
    {
        var svc = new Mock<IProcessoMonitoradoService>();
        svc.Setup(s => s.CreateAsync(It.IsAny<CreateProcessoMonitoradoDto>(), It.IsAny<CancellationToken>()))
           .ThrowsAsync(new InvalidOperationException("Limite atingido."));
        var dataJud = new DataJudAdapter(
            new HttpClient { BaseAddress = new Uri("http://localhost") },
            Mock.Of<ILogger<DataJudAdapter>>());
        var ctrl = new ProcessosMonitoradosController(svc.Object, dataJud, Mock.Of<ILogger<ProcessosMonitoradosController>>());

        var result = await ctrl.Create(new CreateProcessoMonitoradoDto("12345", null), default);
        Assert.IsType<BadRequestObjectResult>(result);
    }
}
